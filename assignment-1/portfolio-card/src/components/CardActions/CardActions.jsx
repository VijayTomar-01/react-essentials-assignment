...
const CardActions = ({darkTheme, handleTheme, totalImages, imageIndex, onPrev, onNext, handleLike, likes, liked, handleAlert}) => {
  ...
  return (
    <div className={`flex flex-wrap justify-evenly items-center pb-4 pt-4 rounded-b-2xl ${darkTheme ? 'bg-gray-700/60' : 'bg-gray-200'}`}>
      <div className=''>
        <button 
        ...
        onClick={handleTheme}>{darkTheme ? '🌞 Light' : '🌛 Dark'}</button>
      </div>
      <div className='flex gap-3 items-center'>
        <button
        className='transition-all duration-300 hover:scale-125 cursor-pointer'
        onClick={onPrev}>◀️</button>
        <p className='text-sm font-semibold'>{imageIndex + 1} / {totalImages}</p>
        <button
        className='transition-all duration-300 hover:scale-125 cursor-pointer'
      ...
      <div className='flex items-center gap-2'>
        ...
        <p className='text-md font-semibold '>{likes}</p>
      </div>
      <div className=''>
        <button
